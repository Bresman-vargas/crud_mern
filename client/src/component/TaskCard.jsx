import { delateTaskRequest } from "../api/tasks.api";
...
export default function TaskCard({index, task}) {
  ...
  return (
    <article
      key={index}
      className="bg-white rounded-md p-6 border border-gray-200 hover:shadow-sm grid  grid-cols-3"
    >
      ...
      <p className="text-gray-700 mb-4 col-span-3">{task.description}</p>
      <div className="flex justify-between items-center text-sm text-gray-600 col-span-2">
        <span
          className={`font-medium rounded-full px-2 border ${
            task.completed
